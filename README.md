...
3. Verify the archive:
   ...
   ```powershell
   $zip = ".\zcode-extensions-v0.3.1-windows-x64.zip"
...
4. Extract the archive to a permanent location. The ZIP contains a stable `zcode-extensions` directory:
   ...
   ```powershell
   Expand-Archive .\zcode-extensions-v0.3.1-windows-x64.zip -DestinationPath D:\
...
```powershell
Expand-Archive .\zcode-extensions-v0.3.1-windows-x64.zip -DestinationPath D:\ -Force
Set-Location D:\zcode-extensions
.\bin\zdp.exe repair
.\bin\zdp.exe launch
```
...
bun run build:sdk
bun run pack:sdk
bun run release:package -- --tag v0.3.1
```
